...
const FixedNavbar = () => {
  ...
  return (
    <header className="pb-5 sm:pt-0 pt-0 w-full">
      <motion.nav
      ...
      >
        <ul className="lg:flex items-center gap-5 hidden">
          {navLinks.map((item) => (
            <div key={item.label} className="relative">
              <Link
                to={`/${item.link}`}
                className="font-melodrama font-extralight text-md text-anon-dark"
              >
                {item.label}
              </Link>
            </div>
        ...
        <Link to="/" onClick={closeMenu}>
          <h1 className="text-4xl text-anon-dark font-melodrama font-bold lg:mr-32">
            Nbvsuxid
          </h1>
        </Link>
        ...
        {/* Mobile Menu Toggle */}
        <button
          className="lg:hidden text-2xl text-anon-dark"
        ...
        <div className="flex sm:gap-3">
          <Bookmark className="text-3.5xl text-anon-dark" />
          <Link
            to="/cart"
            className="flex lg:justify-end justify-center ml-3 relative"
            onClick={closeMenu}
          >
            <Briefcase className="text-3.5xl text-anon-dark" />
            <span className="text-[10px] p-[1px_5px] text-white bg-anon-dark rounded-full absolute -left-1">
              2
            </span>
          </Link>
        </div>
